  ...
  return (
    <section className="aiChatPanel" aria-label="AIチャット">
      ...
      <div className="aiTranscript" ref={transcript} role="log" aria-label="会話履歴" aria-live="off"
        ...
        {!history.length && !pendingUser && <p className="aiWelcome">あら、こんばんは。今日はどうしたのかしら？</p>}
      ...
      <p className="srOnly" aria-live="polite">{busy ? "イアンセが返事をしています。" : history.at(-1)?.content}</p>
      {error && <p className="aiError" role="alert">{error}</p>}
      {!endpoint && <p className="aiNotice">AIモードは準備中です。今はボイス会話でお話しできます。</p>}
      <form className="aiChatForm" onSubmit={send}>
        <label htmlFor="ai-message" className="srOnly">イアンセへのメッセージ</label>
      ...
      <p className="aiNotice">AIモードの入力と直近の会話はGeminiに送信されます。</p>
    </section>
  );
}
