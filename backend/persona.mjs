...
export function buildSystemPrompt(latestMessage, rag = true, useTts = false) {
...
${useTts ? '出力の1行目は必ず [[アニメーション]] の形式で1つだけ。例: [[laugh]]' : '出力の1行目は必ず [[アニメーション|ボイスID]] の形式で1つだけ。例: [[laugh|chuckle]]'}
...
${useTts ? `2行目から話す台詞だけを出力します。台詞全体が音声で読み上げられます。
自然な日本語の話し言葉で、普段は2〜4文程度にまとめて話します。絵文字や読み上げ用の演出指示は書きません。
1文ごとに改行し、最初の文は短くします。完成した文から順番に読み上げが始まります。
相づちや笑い声も台詞に自然に含めます。` : `ボイスは返事の冒頭に短い録音として1回だけ再生されます。以下から会話の文脈と返事の調子に合うIDを1つ選びます。
